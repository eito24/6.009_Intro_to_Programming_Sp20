...
##if list is appended later, the combined_filters function would also change
##because the combined_filters function isn't called yet until it is


# SEAM CARVING

# Main Seam Carving Implementation
def repeated_seaming(new_image):
    ...
    new_image=image_without_seam(new_image,delete_seam)
    return new_image
def seam_carving(image, ncols):
    new_image={'height':image['height'],'width':image['width'],'pixels':list(image['pixels'])}
    for _ in range(ncols):
        new_image=repeated_seaming(new_image)
    return new_image
    """
...
def greyscale_image_from_color_image(image):
    ...
    for i in range(len(image['pixels'])):
        red=.299*image['pixels'][i][0]
        green=.587*image['pixels'][i][1]
        blue=.114*image['pixels'][i][2]
        v=round(red+green+blue)
...
##new get_pixel which if value doesn't exist, returns infinity
...
def image_without_seam(image, removing):
    new_image=image
    x=set(removing)
    for i in len(range(new_image['pixels'])):
        if i not in x:
            new_image['pixels'].append(image['pixels'][i])
    new_image['width']=new_image['width']-1
    return new_image
    """
...
def threshold(threshold_value):
    def thresholding(image):
        ...
        removing=[]
        for i in range(len(image['pixels'])):
            if image['pixels'][i]<threshold_value:
...
if __name__ == '__main__':
    a=load_color_image('test_images/tree.png')
    color_threshold=color_filter_from_greyscale_filter(threshold(100))
    save_color_image(color_threshold(a),'new_tree.png',mode='PNG')
    #save_greyscale_image((b),'new_pattern.png',mode='PNG')
